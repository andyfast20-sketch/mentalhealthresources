...
const pmrSequence = [
  { key: 'feet', label: 'Feet & toes', cue: 'Curl your toes for five seconds, then let them glow and soften.', duration: 15 },
  { key: 'calves', label: 'Calves', cue: 'Press your calves gently into the floor, breathe out, and release.', duration: 15 },
  { key: 'thighs', label: 'Thighs', cue: 'Engage your thighs together, notice the strength, then melt.', duration: 15 },
  { key: 'hips', label: 'Hips & glutes', cue: 'Squeeze through the hips and glutes, then imagine them floating.', duration: 15 },
  { key: 'core', label: 'Belly & lower back', cue: 'Draw your belly in as if hugging your spine, exhale and soften.', duration: 15 },
  { key: 'chest', label: 'Chest & upper back', cue: 'Lift your heart with a sip of air, breathe out and let it settle.', duration: 15 },
  { key: 'hands', label: 'Hands', cue: 'Make soft fists for five counts, then spread your fingers wide.', duration: 15 },
  { key: 'forearms', label: 'Forearms', cue: 'Tense your forearms as if holding a mug, then drop the effort.', duration: 15 },
  { key: 'upper-arms', label: 'Upper arms', cue: 'Gently hug your arms toward your ribs, then loosen completely.', duration: 15 },
  { key: 'shoulders', label: 'Shoulders', cue: 'Lift shoulders toward your ears, slide them down and breathe.', duration: 15 },
  { key: 'neck', label: 'Neck', cue: 'Press your head lightly back, feel the length, then release.', duration: 15 },
  { key: 'face', label: 'Jaw & face', cue: 'Scrunch your brow and jaw, then let your face go serene and soft.', duration: 15 },
];
...
function finishPMR() {
  ...
  if (pmrStart) {
    pmrStart.textContent = 'Start 3-minute flow';
...
function startPMR() {
  ...
  pmrRunning = true;
  pmrIndex = -1;
  pmrEndTime = Date.now() + pmrSequence.reduce((sum, step) => sum + step.duration, 0) * 1000;
  pmrStart.textContent = 'Flow in motion';
  pmrStart.disabled = true;
  if (pmrStatus) {
    pmrStatus.textContent = 'Follow the glow: tense for 5 seconds, then melt for 10.';
  }
  updatePMRCountdown();
  clearInterval(pmrTicker);
...
function resetPMR() {
  ...
  if (pmrCountdown) pmrCountdown.textContent = '03:00';
